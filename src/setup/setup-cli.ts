...
export async function runSetupCommand(args: string[], cwd = process.cwd(), suppliedIo: SetupCommandIo = defaultIo): Promise<number> {
  ...
  try {
    ...
    const checks: Array<[string, string]> = [["Git repository", result.repository.root], ["Remote", result.repository.remote_url], ["Base branch", result.repository.base_branch], ["Project", result.project.kinds.join(", ") || "unknown"]];
    ...
    checks.push(["GitHub", github]);
    checks.push(["ChatGPT Web", result.config.web_bridge?.mode === "actions_relay" ? "connected" : "connects on first task"]);
