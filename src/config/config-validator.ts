...
export function validateConfig(value: unknown): ConfigValidationReport {
  ...
  if (webBridge !== undefined) {
    if (!isRecord(webBridge)) add(issues, "web_bridge must be an object.");
      ...
      if (webBridge.mode !== "personal_actions" && webBridge.mode !== "managed_actions" && webBridge.mode !== "actions_relay" && webBridge.mode !== "manual_file") add(issues, "web_bridge.mode is invalid.");
      ...
      if (webBridge.mode === "personal_actions" && webBridge.relay_url !== undefined && !safeWebUrl(webBridge.relay_url)) add(issues, "web_bridge.relay_url must be HTTPS or an HTTP loopback URL for personal_actions.");
