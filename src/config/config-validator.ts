...
const VERIFICATION_FIELDS = new Set(["allowed_executables", "allowed_environment_keys", "maximum_command_seconds", "maximum_output_bytes", "allowed_generated_paths"]);
...
export function validateConfig(value: unknown): ConfigValidationReport {
  ...
  if (verification !== undefined) {
    if (!isRecord(verification)) add(issues, "verification must be an object.");
      ...
      if (Array.isArray(verification.allowed_generated_paths) && verification.allowed_generated_paths.some((entry) => typeof entry !== "string" || path.isAbsolute(entry) || entry.replaceAll("\\", "/").split("/").includes(".."))) add(issues, "verification.allowed_generated_paths contains an unsafe path.");
