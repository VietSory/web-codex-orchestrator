...
async function installBackup(options: { stateDirectory: string; receipt: ExecutorReceipt; operation: WebImplementationOperation; bytes: Buffer }): Promise<string> {
  ...
  await ensureSecureExecutorSubdirectory(paths.directory, paths.backups);
...
function baseReceipt(options: { runId: string; taskId: string; taskBundleSha256: string; artifactSha256: string; pack: WebImplementationPack; repositoryId: string; baseBranch: string; baseCommit: string; baseTreeSha: string; worktreePath: string; registrationManifestSha256: string; now: () => Date }): ExecutorReceipt {
  ...
  return {
    executor_version: "1.0", run_id: options.runId, task_id: options.taskId, task_bundle_sha256: options.taskBundleSha256, artifact_sha256: options.artifactSha256, pack_id: options.pack.manifest.pack_id,
    state: "VALIDATING", repository_id: options.repositoryId, base_branch: options.baseBranch, base_commit: options.baseCommit, base_tree_sha: options.baseTreeSha, worktree_path: options.worktreePath, registration_manifest_sha256: options.registrationManifestSha256,
    operations: [], change_set_digest: null,
    ...
    errors: [], created_at: timestamp, updated_at: timestamp,
  };
}

export async function prepareExecutorTransaction(options: {
  stateDirectory: string; runId: string; taskId: string; taskBundleSha256: string; artifactSha256: string; pack: WebImplementationPack; repositoryId: string; baseBranch: string; baseCommit: string; baseTreeSha: string; worktreePath: string; registrationManifestSha256: string; now?: () => Date;
  ...
  // Critical invariant: inspect and back up every preimage before the first product write.
  for (const operation of options.pack.operations.operations) {
    ...
    prepared.push({ op_id: operation.op_id, kind: operation.kind, path: operation.path, preimage_sha256: operation.preimage_sha256, postimage_sha256: payload ? sha256(payload) : null, backup_relative_path: backupRelative, backup_sha256: current.sha256, original_mode: current.mode, applied: false });
  }
...
function matchingPackOperation(pack: WebImplementationPack, transaction: ExecutorTransactionOperation): WebImplementationOperation {
  ...
  if (operation.preimage_sha256 !== transaction.preimage_sha256) throw new ExecutorError("EXECUTOR_TRANSACTION_INVALID", `Transaction preimage binding drifted for '${transaction.path}'.`);
  const payload = payloadFor(pack, operation);
  const expectedPost = payload ? sha256(payload) : null;
  if (expectedPost !== transaction.postimage_sha256) throw new ExecutorError("EXECUTOR_TRANSACTION_INVALID", `Transaction postimage binding drifted for '${transaction.path}'.`);
  return operation;
}
...
export function computeExecutorChangeSetDigest(receipt: ExecutorReceipt): string {
  ...
  const body = { version: "1.0", run_id: receipt.run_id, artifact_sha256: receipt.artifact_sha256, base_commit: receipt.base_commit, operations: receipt.operations.map((operation) => ({ op_id: operation.op_id, kind: operation.kind, path: operation.path, preimage_sha256: operation.preimage_sha256, postimage_sha256: operation.postimage_sha256 })).sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0) };
