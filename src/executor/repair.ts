...
async function assertProposalPreimages(receipt: ExecutorReceipt, operations: ReviewerRepairOperation[]): Promise<void> {
  ...
  for (const operation of operations) {
    ...
    if (await classifyRepair(receipt, operation) !== "preimage") throw new ExecutorError("EXECUTOR_REPAIR_INVALID", `Repair preimage is stale or ambiguous at '${operation.path}'.`);
  }
}
...
function archiveVerifiedWebRepair(receipt: ExecutorReceipt, now: () => Date): void {
  ...
  delete receipt.repair;
}

/** Bind one Web-owned bounded repair generation; Harness remains sole mutator. */
export async function bindWebReviewRepair(options: { stateDirectory: string; receipt: ExecutorReceipt; sourceChangeSetDigest: string; sourceReviewEvidenceSha256: string; operations: ReviewerRepairOperation[]; now?: () => Date; }): Promise<ExecutorReceipt> {
  ...
  if (
    receipt.review_strategy !== "web" || receipt.reviewer_selection !== undefined ||
    ...
    (previous && previous.final_change_set_digest !== options.sourceChangeSetDigest) ||
  ...
  await assertProposalPreimages(receipt, options.operations);
  receipt.repair = { reviewer: "web", source_change_set_digest: options.sourceChangeSetDigest, source_review_evidence_sha256: options.sourceReviewEvidenceSha256, operations: options.operations, state: "PROPOSED", final_change_set_digest: null };
...
async function rollbackRepairs(receipt: ExecutorReceipt, pack: WebImplementationPack): Promise<void> {
  ...
  for (const operation of [...receipt.repair.operations].reverse()) { const classification = await classifyRepair(receipt, operation); if (classification === "postimage") await restoreOriginalPostimage(receipt, pack, operation); else if (classification === "ambiguous") throw new ExecutorError("EXECUTOR_AMBIGUOUS_RECOVERY", `Repair rollback found an ambiguous target at '${operation.path}'.`); }
}
...
export async function applyReviewerRepair(options: { stateDirectory: string; receipt: ExecutorReceipt; pack: WebImplementationPack; now?: () => Date }): Promise<ExecutorReceipt> {
  ...
  try {
    ...
    try { await rollbackRepairs(receipt, options.pack); } catch (rollbackError) { throw new ExecutorError("EXECUTOR_AMBIGUOUS_RECOVERY", `Adaptive repair failed and rollback could not restore Web-authorized postimages: ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}.`); }
