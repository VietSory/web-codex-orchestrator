  ...
  return {
    ...
    async review(request: ExecutorReviewRequest) {
      ...
      const result = reviewMode.kind === "terra"
        ? await reviewWithTerra(agentClient, { model: reviewMode.model, reasoning_effort: reviewMode.reasoning_effort, prompt, threadId: undefined, workspacePath: request.worktree_path, acceptedBundlePath: request.accepted_bundle_path, deletedPaths: request.deleted_paths ?? [], ...(request.signal ? { signal: request.signal } : {}) })
        : await reviewWithSol(agentClient, { model: reviewMode.model, reasoning_effort: reviewMode.reasoning_effort, prompt, threadId: undefined, workspacePath: request.worktree_path, acceptedBundlePath: request.accepted_bundle_path, deletedPaths: request.deleted_paths ?? [], ...(request.signal ? { signal: request.signal } : {}) });
