...
import type {
  ResultBundleReceipt, ResultBundleLimits, ResultBundleManifest,
  ManifestEntry, PullRequestAttestation,
} from "./contracts.js";
...
export interface Phase6Options {
   ...
   * The caller supplies exact bytes from an already re-attested executor
   * receipt. Phase 5 publish/PR authority is stable-read again here.
   */
...
export async function packageResultBundle(options: Phase6Options): Promise<ResultBundleReceipt> {
  ...
  if (!/^[A-Za-z0-9_-]{1,128}$/.test(taskId)) throw new ResultBundleError("RESULT_REQUEST_INVALID", "Invalid taskId format.");
  if (!/^[0-9a-f]{64}$/.test(archiveSha)) throw new ResultBundleError("RESULT_REQUEST_INVALID", "Invalid archiveSha format.");
...
async function _buildResultBundle(ctx: {
  ...
  const p5aSnapshot = await readGitPublishReceiptSnapshot(p5aPath);
  if (!p5aSnapshot) throw new ResultBundleError("RESULT_PUBLISH_RECEIPT_INVALID", "Phase 5A receipt not found.");
  const p5aRaw = p5aSnapshot.receipt;
  if (p5aRaw.state !== "PUSHED") throw new ResultBundleError("RESULT_PUBLISH_NOT_PUSHED", `Phase 5A state is '${String(p5aRaw.state)}', expected PUSHED.`);
  if (p5aRaw.run_id !== runId) throw new ResultBundleError("RESULT_PUBLISH_RECEIPT_INCONSISTENT", "Phase 5A run_id mismatch.");
  if (p5aRaw.change_set_sha256 !== changeSetSha256) throw new ResultBundleError("RESULT_PUBLISH_RECEIPT_INCONSISTENT", "Phase 5A change_set_sha256 mismatch.");
  if (p5aRaw.base_commit !== baseCommit) throw new ResultBundleError("RESULT_PUBLISH_RECEIPT_INCONSISTENT", "Phase 5A base_commit mismatch.");
  const publishedCommitSha = String(p5aRaw.commit_sha ?? "");
  const remoteBranchSha = String(p5aRaw.remote_branch_sha ?? "");
  if (!publishedCommitSha || publishedCommitSha !== remoteBranchSha) throw new ResultBundleError("RESULT_REMOTE_SHA_MISMATCH", "Published commit SHA does not match remote branch SHA.");
  const gitPublishReceiptSha256 = sha256Hex(p5aSnapshot.bytes);
  ...
  const p5bSnapshot = await readDraftPullRequestReceiptSnapshot(p5bPath);
  if (!p5bSnapshot) throw new ResultBundleError("RESULT_PR_RECEIPT_INVALID", "Phase 5B receipt not found.");
  const p5bRaw = p5bSnapshot.receipt;
  if (p5bRaw.state !== "OPEN") throw new ResultBundleError("RESULT_PR_NOT_OPEN", `Phase 5B state is '${String(p5bRaw.state)}', expected OPEN.`);
  if (p5bRaw.run_id !== runId) throw new ResultBundleError("RESULT_PR_RECEIPT_INCONSISTENT", "Phase 5B run_id mismatch.");
  ...
  const headBranch = p5aRaw.branch_name;
  const baseBranch = String((executionRaw as Record<string, unknown>).base_branch ?? p5aRaw.base_commit ?? "main");
  const prAttestation: PullRequestAttestation = await attestGitHubPullRequest(
    githubClient,
    repositoryIdentity.owner,
    repositoryIdentity.repository,
    prNumber,
    { headBranch, headSha: publishedCommitSha, baseBranch: p5bRaw.base_branch || baseBranch }
  );
  ...
  const addJson = (entryPath: string, data: unknown): void => {
    ...
    if (found) throw new ResultBundleError("RESULT_SENSITIVE_VALUE_DETECTED", `Secret found in entry '${entryPath}'.`);
  ...
  const addText = (entryPath: string, text: string): void => addBuffer(entryPath, Buffer.from(text.replace(/\r\n/g, "\n"), "utf8"));

  addText("RESULT.md", [
    "# Result Bundle", "", `Run ID: \`${runId}\``, `Task ID: \`${taskId}\``,
    `Published commit: \`${publishedCommitSha}\``, `Pull Request: #${prNumber} (${prAttestation.url})`,
    `Base commit: \`${baseCommit}\``, `Change set: \`${changeSetSha256}\``, "",
    "This archive was produced deterministically by `wco package-result`.",
    "It is sealed for offline Web review.",
  ].join("\n") + "\n");
  addText("REVIEW.md", [
    "# Review Instructions", "", "This Result Bundle is sealed for Web review.",
  ...
  const specLockInputFiles = [...taskFiles]
    ...
    .map(f => ({ path: `task/${f.name}`, sha256: sha256Hex(f.buffer), size_bytes: f.buffer.byteLength }));
  const specSetSha256 = sha256Hex(canonicalJsonBuffer(specLockInputFiles));
  addBuffer("task/spec-lock.json", canonicalJsonBuffer({
    lock_version: "1.0", task_id: taskId, task_archive_sha256: archiveSha,
    accepted_bundle_tree_sha256: acceptedBundleTreeSha256,
    authoritative_files: specLockInputFiles, spec_set_sha256: specSetSha256,
  }));

  addJson("evidence/execution.json", publicExecution);
  addJson("evidence/acceptance.json", { run_id: runId, accepted: publicExecution.verification.required_commands_passed, change_set_sha256: changeSetSha256 });
  addJson("evidence/verification.json", publicVerification);
  addJson("evidence/terra-review.json", {
    run_id: runId, verdict: publicExecution.internal_reviewer.verdict,
  ...
  addJson("evidence/sol-review.json", {
    run_id: runId, verdict: publicExecution.final_reviewer.verdict,
  ...
  addJson("evidence/event-summary.json", {
    run_id: runId, task_id: taskId, base_commit: baseCommit, published_commit_sha: publishedCommitSha,
    change_set_sha256: changeSetSha256, pull_request_number: prNumber, warnings,
  });
  ...
  for (const [filePath, content] of gitEvidence.sourceFiles) addBuffer(`${SOURCE_ENTRY_PREFIX}${filePath}`, content);
  allEntries.sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0);

  const checksumEntries = allEntries.map((entry) => ({ path: entry.path, sha256: sha256Hex(entry.content), size_bytes: entry.content.byteLength }));
  const checksumsJson = canonicalJsonBuffer({ algorithm: "sha256", files: checksumEntries });
  const manifestEntryList: ManifestEntry[] = allEntries.map((entry) => ({ path: entry.path, sha256: sha256Hex(entry.content), size_bytes: entry.content.byteLength }));
  manifestEntryList.push({ path: "checksums.json", sha256: sha256Hex(checksumsJson), size_bytes: checksumsJson.byteLength });
  ...
  const manifest: ResultBundleManifest = {
    schema_version: "1.1", kind: "wco-result-bundle", run_id: runId, archive_filename: archiveFilename,
    published_commit_sha: publishedCommitSha, base_commit: baseCommit, change_set_sha256: changeSetSha256,
    pull_request_number: prNumber, task_id: taskId, created_at: String(executionRaw.created_at ?? createdAt),
    spec_set_sha256: specSetSha256, review_contract_sha256: review_contract_sha256,
    review_policy_sha256: review_policy_sha256, verdict_schema_sha256: verdict_schema_sha256,
    revision_request_schema_sha256: revision_request_schema_sha256, reviewed_entry_set_sha256: reviewedEntrySetSha256,
    entries: manifestEntryList,
  };
  const manifestBuffer = canonicalJsonBuffer(manifest);
  const manifestSha256 = sha256Hex(manifestBuffer);
  const finalEntries: ZipEntry[] = [...allEntries, { path: "checksums.json", content: checksumsJson }, { path: "manifest.json", content: manifestBuffer }];
  finalEntries.sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0);
  if (finalEntries.length > limits.maximum_entries) throw new ResultBundleError("RESULT_ARCHIVE_ENTRY_LIMIT", `Too many entries: ${finalEntries.length}`);

  const input_digest_sha256 = sha256Hex(Buffer.from(executionReceiptSha256 + gitPublishReceiptSha256 + draftPrReceiptSha256, "utf8"));
  const baseReceipt: ResultBundleReceipt = {
    result_bundle_version: "1.1", run_id: runId, state: "READY_TO_BUILD", input_digest_sha256,
    execution_receipt_sha256: executionReceiptSha256, git_publish_receipt_sha256: gitPublishReceiptSha256,
    draft_pr_receipt_sha256: draftPrReceiptSha256, accepted_bundle_tree_sha256: acceptedBundleTreeSha256,
    reviewed_entry_set_sha256: reviewedEntrySetSha256, change_set_sha256: changeSetSha256, base_commit: baseCommit,
    published_commit_sha: publishedCommitSha, remote_branch_sha: remoteBranchSha, pull_request: prAttestation,
    archive_relative_path: null, archive_sha256: null, archive_size_bytes: null, entry_count: null,
    uncompressed_size_bytes: null, manifest_sha256: manifestSha256, warnings, created_at: createdAt,
    updated_at: currentIso(now), built_at: null, verified_at: null, ready_at: null,
    spec_set_sha256: specSetSha256, review_contract_sha256: review_contract_sha256,
    review_policy_sha256: review_policy_sha256, verdict_schema_sha256: verdict_schema_sha256,
  ...
  const builtArchive = await buildDeterministicZip(finalEntries, paths.directory, archiveFilename, {
...
export async function getResultBundleStatus(options: { runId: string; stateDirectory: string }): Promise<ResultBundleReceipt | null> {
  ...
  return readResultBundleReceipt(resultBundlePaths(resolvedStateDir, taskId, archiveSha).receiptPath);
}
