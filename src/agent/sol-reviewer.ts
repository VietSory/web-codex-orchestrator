...
export async function reviewWithSol(client: AgentClient, request: { model: string; reasoning_effort: "minimal" | "low" | "medium" | "high" | "xhigh"; prompt: string; threadId: string | undefined; workspacePath: string; acceptedBundlePath: string; deletedPaths?: string[]; signal?: AbortSignal | undefined }): Promise<{ threadId: string; review: ReviewResult; response: Awaited<ReturnType<AgentClient["turn"]>> }> {
  ...
  await assertSeniorReviewFindingLocations(review, request.workspacePath, request.deletedPaths ?? []);
