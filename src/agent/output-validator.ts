...
export function validateReview(output: unknown): ReviewResult {
  const value = parseAgentJson(output);
  if (!record(value) || !exactKeys(value, ["verdict", "reviewed_change_set_sha256", "summary", "acceptance_results", "blocking_findings", "non_blocking_findings", "scope_violations", "unverified_acceptance", "recommended_next_state", "human_action"]) || !["APPROVE", "REVISE", "REPLAN", "ESCALATE"].includes(String(value.verdict)) || typeof value.reviewed_change_set_sha256 !== "string" || !/^[0-9a-f]{64}$/.test(value.reviewed_change_set_sha256) || typeof value.summary !== "string" || value.summary.length > 16_384 || !Array.isArray(value.acceptance_results) || value.acceptance_results.length > 512 || !Array.isArray(value.blocking_findings) || value.blocking_findings.length > 256 || !value.blocking_findings.every(finding) || !Array.isArray(value.non_blocking_findings) || value.non_blocking_findings.length > 256 || !value.non_blocking_findings.every(finding) || !stringArray(value.scope_violations) || !stringArray(value.unverified_acceptance) || !(value.human_action === null || record(value.human_action))) throw new ExecutionError("REVIEW_OUTPUT_INVALID", "Invalid reviewer output.");
