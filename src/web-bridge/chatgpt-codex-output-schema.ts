 ...
 * never author implementation/mutation authority. An AUTHOR_CONTEXT recovery
 * phase is repository-command-only when an exact durable lookup is too large
 * to retransmit safely in one semantic follow-up. Independent code review has
 ...
 * request more bounded context or return a verdict. None of these phases
 * receives shell, network, Git, publish, mutation, or merge authority. The
 * blind challenger may only request bounded repository context or seal a
 * non-authoritative understanding.
 */
export const CHATGPT_CODEX_AUTHOR_KINDS = ["repository_command", "contract_sealed"] as const;
export const CHATGPT_CODEX_AUTHOR_CONTEXT_KINDS = ["repository_command"] as const;
...
export const CHATGPT_CODEX_AUTHOR_CONTEXT_OUTPUT_SCHEMA = providerSchema(CHATGPT_CODEX_AUTHOR_CONTEXT_KINDS);
