...
export async function materializeWebImplementationPack(options: { submission: WebImplementationSubmission | unknown; envelope: WebContractEnvelope | unknown; stateDirectory: string; configPath: string; coverageStore?: ReadCoverageStore }): Promise<MaterializedWebPack> {
  ...
  const checksumEntries = [...entries.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([entryPath, content]) => ({ path: entryPath, sha256: sha256(content), size_bytes: content.byteLength }));
  ...
  const archive = await buildWebBridgeZip([...entries.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([entryPath, content]) => ({ path: entryPath, content })), output, "web-implementation-pack.zip", { maximumEntries: 512, maximumArchiveBytes: 33_554_432, maximumUncompressedBytes: 67_108_864 });
