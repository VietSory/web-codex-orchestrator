...
export class FakeWebBridge implements WebBridge {
  ...
  async createAuthoringJob(request: AuthoringJobRequest, idempotencyKey: string): Promise<BridgeJobIdentity> { return this.create(this.authoring, "job", request, idempotencyKey, request.owner, request.ttl_seconds); }
