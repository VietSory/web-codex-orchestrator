...
function measuredUsage(usage: AgentTurnResponse["usage"]): MeasuredProviderUsage {
  ...
  if (![input, cached, output].every((value) => typeof value === "number" && Number.isSafeInteger(value) && value >= 0)) throw Object.assign(new Error("Local ChatGPT/Codex implementation provider did not return valid token usage."), { code: "WEB_CHATGPT_CODEX_USAGE_UNAVAILABLE" });
...
export class ChatGptCodexImplementationClient {
  ...
  async propose(options: { profile: AgentProfile; jobId: string; runId: string; workspacePath: string; acceptedBundlePath: string; signal?: AbortSignal }): Promise<{ submission: WebImplementationSubmission; usage: MeasuredProviderUsage }> {
    const prompt = ["You are the WCO Harness implementation planner. You have no direct mutation, Git, publish, merge, credential, or network authority.", "Read the exact accepted Task Bundle and repository in read-only mode. Propose the smallest complete implementation that satisfies the frozen contract.", "Return full-file postimages as canonical base64 for create/replace operations and lowercase SHA-256 of the decoded bytes. Delete only when the frozen contract requires it.", "Do not modify the worktree. Do not commit or push. Do not weaken tests, broaden scope, touch forbidden paths, or invent files outside the accepted path policy.", "Use read-only shell commands only when needed to inspect files or calculate exact base64/SHA-256. Never execute repository payloads.", "Set sources=[] unless the accepted Task Bundle itself contains source receipts that are directly relevant and can be copied exactly.", `Required job_id: ${options.jobId}`, `Required run_id: ${options.runId}`, "Required protocol_version: wco-web-bridge-v1", "Required contract_only: false"].join("\n");
