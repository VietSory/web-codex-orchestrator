...
export class ChatGptCodexSemanticChallengeTransport implements SemanticChallengeTransport {
  ...
  private recordUsage(state: ChallengeState, usage: Usage): void {
    if (usage.cached_input_tokens > usage.input_tokens) throw new Error("semantic challenge cached input usage exceeds total input usage.");
    state.usage.turns = safeAdd(state.usage.turns, 1);
    state.usage.input_tokens = safeAdd(state.usage.input_tokens, usage.input_tokens);
    state.usage.cached_input_tokens = safeAdd(state.usage.cached_input_tokens, usage.cached_input_tokens);
    state.usage.output_tokens = safeAdd(state.usage.output_tokens, usage.output_tokens);
  ...
  async waitForSemanticChallengeAction(jobId: string, afterSequence: number, signal?: AbortSignal): Promise<SemanticChallengeRemoteAction | null> {
    ...
    const prompt = state.pending_result ? followupPrompt(state.pending_result.request_id, state.pending_result) : wirePrompt(state.request);
