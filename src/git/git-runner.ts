...
export class GitRunner {
  ...
  private getCommandTarget(args: readonly string[]): { subcommand: string | undefined; varTarget: string | undefined } {
    ...
    while (i < args.length) {
      ...
      if (arg === "-c") { i += 2; continue; }
      if (arg === "--literal-pathspecs") { i += 1; continue; }
      if (arg.startsWith("-")) return { subcommand: undefined, varTarget: undefined };
  ...
  private safeEnvironment(subcommand: string | undefined, varTarget: string | undefined): NodeJS.ProcessEnv {
    const result: NodeJS.ProcessEnv = { GIT_TERMINAL_PROMPT: "0", GIT_CONFIG_NOSYSTEM: "1" };
    if (this.runtimeDirectory !== undefined) result.GIT_CONFIG_GLOBAL = path.join(this.runtimeDirectory, "empty-config");
    for (const key of ["PATH", "HOME", "USERPROFILE", "SYSTEMROOT", "TMPDIR", "TEMP", "TMP", "LANG", "LC_ALL", "WCO_GIT_EXECUTABLE"]) {
      const value = this.env[key];
      if (value !== undefined) result[key] = value;
    }
    if (this.security?.identity && (subcommand === "commit" || (subcommand === "var" && (varTarget === "GIT_AUTHOR_IDENT" || varTarget === "GIT_COMMITTER_IDENT")))) {
  ...
  async run(args: readonly string[], cwd: string): Promise<GitCommandResult> {
    ...
    const effectiveArgs = this.runtimeDirectory === undefined
      ? [...args]
      : ["-c", `core.hooksPath=${path.join(this.runtimeDirectory, "empty-hooks")}`, "-c", "core.fsmonitor=false", ...args];
    const env = this.safeEnvironment(subcommand, varTarget);
    const result = await spawnBounded({
      executable: env.WCO_GIT_EXECUTABLE || "git",
      args: effectiveArgs,
      cwd,
      shell: false,
    ...
    const boundedFailure = Boolean(result.spawnError || result.timedOut || result.cancelled || result.stdoutTruncated || result.stderrTruncated);
    return {
      executable: "git",
      args: effectiveArgs,
      cwd,
