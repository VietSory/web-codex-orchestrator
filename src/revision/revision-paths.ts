...
function assertContained(root: string, target: string): void {
  ...
  if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
    throw new RevisionError("REVISION_STATE_UNSAFE", `Revision path escapes state directory: ${target}`);
  }
}
...
async function assertRealDirectory(directory: string): Promise<void> {
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory()) {
    throw new RevisionError("REVISION_STATE_UNSAFE", `Revision lifecycle path is not a real directory: ${directory}`);
  }
}
...
export function resolveRevisionRoundPaths(stateDirectory: string, runId: string, round: number): RevisionRoundPaths {
  if (!Number.isInteger(round) || round < 1 || round > 3) {
    throw new RevisionError("REVISION_REQUEST_INVALID", `Revision round must be an integer between 1 and 3; got ${round}`);
  }
  ...
  assertContained(root, resultDirectory);
  return {
    directory,
...
export async function prepareRevisionRoundPaths(stateDirectory: string, paths: RevisionRoundPaths): Promise<void> {
  await ensurePath(stateDirectory, paths.directory);
  await ensurePath(stateDirectory, paths.resultDirectory);
}

export async function assertExistingRevisionPathSafe(
