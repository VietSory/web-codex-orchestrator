...
async function readBounded(filePath: string, missingAllowed: boolean): Promise<Buffer | null> {
  ...
  if (before.isSymbolicLink() || !before.isFile()) {
    throw new RevisionError("REVISION_STATE_UNSAFE", `Revision artifact must be a regular non-symlink file: ${filePath}`);
  }
  if (before.size > MAX_REVISION_ARTIFACT_BYTES) {
    throw new RevisionError("REVISION_STATE_INVALID", `Revision artifact exceeds ${MAX_REVISION_ARTIFACT_BYTES} bytes: ${filePath}`);
  }
  ...
  try {
    ...
    if (!opened.isFile() || opened.dev !== before.dev || opened.ino !== before.ino || opened.size !== before.size) {
      throw new RevisionError("REVISION_STATE_INVALID", `Revision artifact changed during open: ${filePath}`);
    }
    ...
    if (after.dev !== opened.dev || after.ino !== opened.ino || after.size !== opened.size) {
      throw new RevisionError("REVISION_STATE_INVALID", `Revision artifact changed during read: ${filePath}`);
    }
    const pathAfter = await fs.lstat(filePath);
    if (pathAfter.isSymbolicLink() || !pathAfter.isFile() || pathAfter.dev !== opened.dev || pathAfter.ino !== opened.ino || pathAfter.size !== opened.size) {
      throw new RevisionError("REVISION_STATE_INVALID", `Revision artifact path changed during read: ${filePath}`);
    }
    return buffer;
  } finally {
...
export function assertRevisionReceipt(value: unknown): asserts value is RevisionReceipt {
  ...
  for (const field of ["spec_set_sha256", "revision_request_sha256", "previous_result_bundle_sha256", "previous_verdict_sha256"] as const) {
    if (typeof obj[field] !== "string" || !/^[a-f0-9]{64}$/.test(obj[field] as string)) throw new RevisionError("REVISION_STATE_INVALID", `${field} must be a 64-hex SHA-256.`);
  }
  for (const field of ["previous_published_commit_sha", "previous_pr_head_sha"] as const) {
    if (typeof obj[field] !== "string" || !/^[a-f0-9]{40}$/.test(obj[field] as string)) throw new RevisionError("REVISION_STATE_INVALID", `${field} must be a 40-hex commit SHA.`);
  }
  ...
  if (!Array.isArray(obj.revision_paths) || obj.revision_paths.some((entry) => typeof entry !== "string")) throw new RevisionError("REVISION_STATE_INVALID", "revision_paths is invalid.");
