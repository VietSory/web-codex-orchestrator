...
function validateInventory(pack: WebImplementationPack): void {
  ...
  for (const raw of doc.entries) {
    ...
    if (seen.has(entry.path) || previous !== undefined && previous >= entry.path) throw new WebAuthorityError("WEB_AUTHORITY_MANIFEST_INVALID", "Repository inventory must be unique and strictly lexical.");
    seen.add(entry.path); previous = entry.path;
  }
}

function validateReadCoverage(pack: WebImplementationPack): void {
  ...
  for (const raw of doc.reads) {
    ...
    if (!GIT_SHA.test(String(read.object_sha)) || !["full", "partial"].includes(String(read.coverage)) || seen.has(read.path)) throw new WebAuthorityError("WEB_AUTHORITY_MANIFEST_INVALID", `Invalid/duplicate read coverage for '${read.path}'.`);
    seen.add(read.path);
  }
}

function validateProjectMap(pack: WebImplementationPack): void {
...
export function validateWebImplementationPackSemantics(pack: WebImplementationPack): void {
  validateInventory(pack);
  validateReadCoverage(pack);
  validateProjectMap(pack);
