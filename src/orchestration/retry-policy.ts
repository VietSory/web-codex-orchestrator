...
function validateRetryOptions(options: RetryOptions): void {
  for (const [name, value] of Object.entries(options)) if (!Number.isSafeInteger(value) || value < 1 || value > 24 * 60 * 60 * 1000) throw new OrchestrationError("ORCHESTRATION_RETRY_INVALID", `Retry option '${name}' is outside bounded integer policy.`);
  if (options.base_delay_ms > options.maximum_delay_ms) throw new OrchestrationError("ORCHESTRATION_RETRY_INVALID", "base_delay_ms must not exceed maximum_delay_ms.");
}
...
export function retryableFailureCode(code: string): boolean { return /(?:^|_)(?:TIMEOUT|NETWORK|RATE_LIMIT|TEMPORARY|UNAVAILABLE|DISCONNECTED|RETRYABLE)(?:_|$)/.test(code); }
export function decideRetry(ledger: RunLedger, failureCode: string, now: Date, options: RetryOptions = DEFAULT_RETRY_OPTIONS): RetryDecision {
  ...
  const elapsed = now.getTime() - Date.parse(ledger.budget.started_at); if (!Number.isFinite(elapsed) || elapsed > ledger.budget.max_elapsed_ms) return { retry: false, delay_ms: 0, reason: "elapsed budget exhausted" }; if (ledger.retry.consecutive_failures >= options.maximum_consecutive_failures) return { retry: false, delay_ms: options.circuit_open_ms, reason: "circuit open" }; return { retry: true, delay_ms: computeRetryDelay(attempt.request_sha256, attempt.attempt_number, options), reason: "retryable" };
}
export function sealTransitionRequest(transition: TransitionKind, payload: unknown): string { return crypto.createHash("sha256").update(canonicalJsonBuffer({ transition, payload })).digest("hex"); }
export function beginAttempt(transition: TransitionKind, requestSha256: string, attemptNumber: number, now: Date): TransitionAttempt { if (!SHA256.test(requestSha256)) throw new OrchestrationError("ORCHESTRATION_REQUEST_INVALID", "Transition request SHA-256 is invalid."); const attemptId = crypto.createHash("sha256").update(`${transition}:${requestSha256}:${attemptNumber}`).digest("hex").slice(0, 32); return { transition, attempt_id: attemptId, request_sha256: requestSha256, status: "STARTED", attempt_number: attemptNumber, started_at: now.toISOString(), finished_at: null, result_sha256: null, failure_code: null }; }
