...
export function deriveNextTransition(snapshot: LifecycleSnapshot): PlannedTransition {
  ...
  if (snapshot.executor_state === "ESCALATE_TO_WEB") return { transition: "REGISTER_WEB_PACK", reason: "The previous registered implementation was rejected; Web authority must provide a new registered pack.", mutating: false, requires_human: false };
  if (snapshot.executor_state === "FAILED") return { transition: "WAIT_HUMAN", reason: "Executor reached a terminal operational failure requiring human recovery.", mutating: false, requires_human: true };
