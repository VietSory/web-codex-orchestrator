...
function emptyAttemptCounters(): TransitionAttemptCounters {
  return Object.fromEntries(TRANSITIONS.map((value) => [value, 0])) as TransitionAttemptCounters;
}

async function prepareDirectory(stateDirectory: string, directory: string): Promise<void> {
...
async function readStableFile(filePath: string): Promise<Buffer> {
  ...
  try {
    ...
    while (offset < bytes.length) {
      ...
      if (bytesRead === 0) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger truncated while reading.");
      offset += bytesRead;
    }
...
function hashEvent(event: Omit<OrchestrationEvent, "event_hash">): string {
  return crypto.createHash("sha256").update(canonicalJsonBuffer(event)).digest("hex");
}

function validateEventChain(ledger: RunLedger): void {
  let previous = ledger.history_anchor_hash;
  let expectedSequence = ledger.compacted_event_count + 1;
...
function validateLedger(ledger: RunLedger): void {
  ...
  if (ledger.current_attempt && (!TRANSITIONS.includes(ledger.current_attempt.transition) || !SHA256.test(ledger.current_attempt.request_sha256) || ledger.current_attempt.result_sha256 !== null && !SHA256.test(ledger.current_attempt.result_sha256) || !Number.isSafeInteger(ledger.current_attempt.attempt_number) || ledger.current_attempt.attempt_number < 1)) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Current transition attempt is invalid.");
...
export function createRunLedger(options: { runId: string; nextTransition?: TransitionKind; now?: Date }): RunLedger {
  ...
  return {
    ledger_version: "1.0", run_id: options.runId, task_id: options.runId.slice(0, split), task_bundle_sha256: options.runId.slice(split + 1), status: "ACTIVE", paused: false, pause_reason: null, next_transition: options.nextTransition ?? "REGISTER_WEB_PACK", current_attempt: null, last_completed_transition: null, transition_attempts: emptyAttemptCounters(),
    budget: { max_attempts_per_transition: 4, max_total_attempts: 24, max_elapsed_ms: 24 * 60 * 60 * 1000, max_model_turns: 32, max_input_tokens: 1_000_000, max_output_tokens: 250_000, total_attempts: 0, model_turns: 0, input_tokens: 0, output_tokens: 0, started_at: now },
    retry: { consecutive_failures: 0, next_retry_at: null, circuit_state: "CLOSED", circuit_opened_at: null, last_failure_code: null }, diagnostics: [], history_anchor_hash: ZERO_HASH, compacted_event_count: 0, events: [], created_at: now, updated_at: now,
  };
}

export function appendLedgerEvent(ledger: RunLedger, kind: string, data: unknown, now: Date): void {
  const dataSha = crypto.createHash("sha256").update(canonicalJsonBuffer(data)).digest("hex");
  const previousHash = ledger.events.at(-1)?.event_hash ?? ledger.history_anchor_hash;
  ...
  while (ledger.events.length > MAX_EVENTS) {
    const removed = ledger.events.shift()!;
    ledger.history_anchor_hash = removed.event_hash;
    ledger.compacted_event_count += 1;
  }
...
export function recordDiagnostic(ledger: RunLedger, code: string, message: string, now: Date): void {
  const normalizedCode = code.slice(0, 128);
  const normalizedMessage = message.slice(0, MAX_DIAGNOSTIC_CHARS);
  const existing = ledger.diagnostics.find((item) => item.code === normalizedCode && item.message === normalizedMessage);
  if (existing) { existing.count += 1; existing.last_at = now.toISOString(); }
  else {
    ledger.diagnostics.push({ code: normalizedCode, message: normalizedMessage, count: 1, first_at: now.toISOString(), last_at: now.toISOString() });
    if (ledger.diagnostics.length > MAX_DIAGNOSTICS) ledger.diagnostics.splice(0, ledger.diagnostics.length - MAX_DIAGNOSTICS);
  }
  ledger.updated_at = now.toISOString();
}
...
export async function readRunLedger(stateDirectory: string, runId: string): Promise<RunLedger | null> {
  ...
  if (split <= 0) throw new OrchestrationError("ORCHESTRATION_RUN_ID_INVALID", "Invalid run_id.");
  const paths = orchestrationPaths(stateDirectory, runId.slice(0, split), runId.slice(split + 1));
  let bytes: Buffer;
  try { bytes = await readStableFile(paths.ledger); }
  catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return null; throw error; }
  let parsed: unknown;
  try { parsed = JSON.parse(bytes.toString("utf8")); } catch { throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger is not valid JSON."); }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger must be an object.");
  const ledger = parsed as RunLedger;
  validateLedger(ledger);
  if (ledger.run_id !== runId) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger path identity does not match its body.");
  return ledger;
}

export async function writeRunLedger(stateDirectory: string, ledger: RunLedger): Promise<void> {
  validateLedger(ledger);
  const paths = orchestrationPaths(stateDirectory, ledger.task_id, ledger.task_bundle_sha256);
  await prepareDirectory(stateDirectory, paths.directory);
  const bytes = canonicalJsonBuffer(ledger);
  if (bytes.byteLength > MAX_LEDGER_BYTES) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger exceeds byte cap.");
  const temp = path.join(paths.directory, `.run-ledger.${process.pid}.${crypto.randomUUID()}.tmp`);
  await fs.writeFile(temp, bytes, { flag: "wx", mode: 0o600 });
  try {
    const existing = await fs.lstat(paths.ledger).catch((error) => (error as NodeJS.ErrnoException).code === "ENOENT" ? null : Promise.reject(error));
    if (existing?.isSymbolicLink()) throw new OrchestrationError("ORCHESTRATION_STATE_INVALID", "Run ledger path is a symlink.");
    await fs.rename(temp, paths.ledger);
