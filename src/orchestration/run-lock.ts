...
async function installAuthorityLock(directory: string, lockPath: string, bytes: Buffer): Promise<StableFileIdentity | null> {
  ...
  try {
    ...
    if (linkedStat.isSymbolicLink() || !linkedStat.isFile() || !sameInode(prepared, linkedStat)) {
    ...
    if (installed.isSymbolicLink() || !installed.isFile() || !sameInode(prepared, installed)) {
      throw new OrchestrationError("ORCHESTRATION_LOCK_INVALID", "Run lock changed after atomic installation.");
    }
    return stableIdentity(installed);
  } catch (error) {
    if (linked && prepared) {
      const current = await fs.lstat(lockPath).catch(() => null);
      if (current && current.isFile() && !current.isSymbolicLink() && sameInode(prepared, current)) {
