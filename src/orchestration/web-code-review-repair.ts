...
/**
 * Apply one bounded PAIR Web repair generation through the shared Harness.
 * This is used by both independent Web-B code review and original Web-A final
 * intent review. The Web supplies exact bytes/preimages but never mutation
 * authority. A different prior generation may be rotated only after VERIFIED;
 * an in-flight different generation always conflicts fail-closed.
 */
export async function applyPairWebRepair(options: { envelope: WebVerdictEnvelope; stateDirectory: string; configPath: string; now?: () => Date; }) {
  ...
  if (receipt.review_strategy !== "web" || receipt.reviewer_selection !== undefined) throw new WebBridgeError("WEB_CODE_REVIEW_REPAIR_INVALID", "Direct Web repair is valid only for the PAIR Web-review strategy.");
  ...
  const repaired = await executeRegisteredWebPack({ runId: options.envelope.run_id, artifactSha256: selected.artifact_sha256, stateDirectory: options.stateDirectory, configPath: options.configPath, verifier, reviewStrategy: "web", ...(options.now ? { now: options.now } : {}) });
...
/** Backward-compatible name for the independent Web-B code-review caller. */
export const applyWebCodeReviewRepair = applyPairWebRepair;