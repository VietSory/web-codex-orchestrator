...
import { runNextTransition, type ContinueInputs } from "./transition-runner.js";
...
function parse(command: string, argv: string[]): CommonArgs {
  ...
  for (const key of values.keys()) {
    if (!["--run-id", "--state-dir", "--config", "--web-pack", "--verdict", "--max-transitions", "--json"].includes(key)) {
  ...
  if (command !== "continue" && (values.has("--web-pack") || values.has("--verdict") || values.has("--max-transitions"))) {
    throw new OrchestrationError("ORCHESTRATION_CLI_INVALID", "--web-pack/--verdict/--max-transitions are valid only for continue.");
  }
  ...
  return {
    ...
    ...(typeof verdict === "string" ? { verdictPath: path.resolve(verdict) } : {}),
...
function basicDoctorProbes(args: CommonArgs): DoctorProbe[] {
  return [
    {
      id: "node",
      async run() { return { severity: Number(process.versions.node.split(".")[0]) >= 20 ? "OK" as const : "FAIL" as const, summary: `Node ${process.versions.node}` }; },
    },
    {
      id: "state",
      async run() { await fs.mkdir(args.stateDirectory, { recursive: true, mode: 0o700 }); await fs.access(args.stateDirectory, fs.constants.R_OK | fs.constants.W_OK); return { severity: "OK" as const, summary: "state directory readable/writable" }; },
    },
    {
      id: "config",
      async run() { const stat = await fs.stat(args.configPath!); return { severity: stat.isFile() ? "OK" as const : "FAIL" as const, summary: stat.isFile() ? "config file present" : "config path is not a file" }; },
    },
    {
      id: "git",
      async run() {
        ...
        if (result.exitCode !== 0 || result.timedOut || result.spawnError) return { severity: "FAIL" as const, summary: result.stderr.trim() || "git unavailable" };
...
export async function runControlCommand(command: string, argv: string[], io: ControlCliIo): Promise<number> {
  try {
    ...
    if (command === "status") {
      const [ledger, snapshot] = await Promise.all([readRunLedger(args.stateDirectory, args.runId), readLifecycleSnapshot(args.stateDirectory, args.runId)]);
      emit(io, args.json, { ledger, snapshot, next: deriveNextTransition(snapshot) });
      return 0;
    }
    if (command === "next") {
      const snapshot = await readLifecycleSnapshot(args.stateDirectory, args.runId);
      emit(io, args.json, deriveNextTransition(snapshot));
      return 0;
    }
    if (command === "pause") { emit(io, args.json, await pauseRun(args.stateDirectory, args.runId, "operator pause")); return 0; }
    if (command === "resume") { emit(io, args.json, await resumeRun(args.stateDirectory, args.runId)); return 0; }
    if (command === "doctor") {
      const report = await runDoctor(basicDoctorProbes(args));
      emit(io, args.json, report);
      return report.status === "FAIL" ? 2 : 0;
    }
    if (command === "continue") {
      let latest: Awaited<ReturnType<typeof runNextTransition>> | null = null;
      let availableWebPackPath = args.webPackPath;
      let availableVerdictPath = args.verdictPath;
      for (let index = 0; index < args.maxTransitions; index += 1) {
        const inputs: ContinueInputs = {};
        if (availableWebPackPath) inputs.web_pack_path = availableWebPackPath;
        if (availableVerdictPath) inputs.verdict_path = availableVerdictPath;
        ...
        if (latest.ledger.last_completed_transition === "REGISTER_WEB_PACK") availableWebPackPath = undefined;
        if (latest.ledger.last_completed_transition === "WAIT_WEB_VERDICT") availableVerdictPath = undefined;
        if (!latest.progressed || latest.needs_input || ["WAIT_HUMAN", "REVISE", "DONE"].includes(latest.planned.transition)) break;
      }
      emit(io, args.json, latest ?? { progressed: false, message: "no transition executed" });
      return 0;
    }
