...
export async function runNextTransition(options: {
  ...
  return await withTransitionExecutionLock(options.stateDirectory, options.runId, async () => {
    ...
    if (ledger.paused) {
      return { ledger, planned, progressed: false, needs_input: "resume" };
    }
    if (["WAIT_WEB_VERDICT", "WAIT_HUMAN", "DONE", "OPEN_DRAFT_PR", "PACKAGE_RESULT", "REVISE"].includes(planned.transition)) {
      ...
      if (planned.transition === "REGISTER_WEB_PACK") {
        ...
        if (registration.artifact_sha256 !== pack.archive_sha256) {
          throw new OrchestrationError(
            "ORCHESTRATION_ARTIFACT_DRIFT",
            "Registered Web pack SHA differs from the sealed transition request.",
          );
        }
        ...
        if (!registration) {
          throw new OrchestrationError(
            ...
            "No selected registered Web pack exists.",
          );
        }
        ledger = await checkpointAttempt({
          stateDirectory: options.stateDirectory,
        ...
        if (
          ...
          throw new OrchestrationError(
            "ORCHESTRATION_PUBLISH_INCOMPLETE",
            "Publication ended without an exact verified PUSHED commit.",
          );
        }
        ledger = await completeAttempt({
          stateDirectory: options.stateDirectory,
    ...
    const blockedByFailure = operationFailed || ["WAITING", "BLOCKED", "PAUSED", "FAILED"].includes(ledger.status);
    return {
      ledger,
      planned: afterPlan,
      progressed: !blockedByFailure,
      needs_input: !blockedByFailure && afterPlan.transition === "REGISTER_WEB_PACK" ? "web_pack_path" : null,
    };
  });
}
