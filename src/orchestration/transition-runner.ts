...
export interface ContinueInputs { web_pack_path?: string; web_verdict_path?: string; }
...
function errorRetryAfterMs(error: unknown): number | undefined { if (!error || typeof error !== "object" || !("retryAfterMs" in error)) return undefined; const value = (error as { retryAfterMs?: unknown }).retryAfterMs; return Number.isSafeInteger(value) && typeof value === "number" && value > 0 ? value : undefined; }
...
export async function runNextTransition(options: { runId: string; stateDirectory: string; configPath: string; inputs?: ContinueInputs; dependencies?: Partial<OrchestrationDependencies>; now?: () => Date }): Promise<ContinueResult> {
  return await withTransitionExecutionLock(options.stateDirectory, options.runId, async () => {
    ...
    if (planned.transition === "WAIT_WEB_VERDICT" && !options.inputs?.web_verdict_path) return { ledger, planned, progressed: false, needs_input: "web_verdict_path" };
    ...
    try {
      ...
      else if (planned.transition === "PACKAGE_RESULT") { const registration = await deps.readSelectedArtifact(options.stateDirectory, options.runId); if (!registration) throw new OrchestrationError("ORCHESTRATION_ARTIFACT_INVALID", "No selected registered Web pack exists."); const ready = await deps.attestReadyExecutor({ runId: options.runId, artifactSha256: registration.artifact_sha256, stateDirectory: options.stateDirectory, configPath: options.configPath }); ledger = await checkpointAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, transition: "PACKAGE_RESULT", payload: { artifact_sha256: registration.artifact_sha256, change_set_digest: ready.changeSetDigest }, now: now() }); activeAttemptId = ledger.current_attempt!.attempt_id; const result = await deps.packageResult({ runId: options.runId, stateDirectory: options.stateDirectory, configPath: options.configPath, now }); if (result.state !== "READY_FOR_WEB_REVIEW" || result.run_id !== options.runId || result.archive_sha256 === null || result.published_commit_sha !== result.remote_branch_sha || result.pull_request.draft !== true || result.pull_request.head_sha !== result.published_commit_sha) throw new OrchestrationError("ORCHESTRATION_RESULT_INCOMPLETE", "Result Bundle operation ended without an exact verified Draft-PR-bound handoff."); ledger = await completeAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, attemptId: activeAttemptId, result: { state: result.state, archive_sha256: result.archive_sha256, published_commit_sha: result.published_commit_sha, pull_number: result.pull_request.number, reviewed_entry_set_sha256: result.reviewed_entry_set_sha256 }, nextTransition: "WAIT_WEB_VERDICT", now: now() }); }
      else if (planned.transition === "WAIT_WEB_VERDICT") { const ingested = await deps.readVerdict(options.inputs!.web_verdict_path!); ledger = await checkpointAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, transition: "WAIT_WEB_VERDICT", payload: { verdict_sha256: ingested.verdictSha256 }, now: now() }); activeAttemptId = ledger.current_attempt!.attempt_id; const review = await deps.submitWebVerdict({ runId: options.runId, stateDirectory: options.stateDirectory, configPath: options.configPath, ingestedVerdict: ingested, now }); if (review.verdict_sha256 !== ingested.verdictSha256 || review.run_id !== options.runId || !["APPROVED", "REVISION_REQUESTED", "ESCALATED"].includes(review.state) || review.fresh_attested_head_sha === null || review.fresh_attested_head_sha !== review.published_commit_sha || review.decision_event_sha256 === null) throw new OrchestrationError("ORCHESTRATION_WEB_VERDICT_INCOMPLETE", "Web verdict processing did not produce an exact sealed terminal decision for the freshly attested Draft PR head."); const next = review.state === "REVISION_REQUESTED" ? "REVISE" : "WAIT_HUMAN"; ledger = await completeAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, attemptId: activeAttemptId, result: { state: review.state, review_round: review.review_round, verdict_sha256: review.verdict_sha256, decision_event_sha256: review.decision_event_sha256, revision_request_sha256: review.revision_request_sha256, published_commit_sha: review.published_commit_sha, pull_request_number: review.pull_request_number, fresh_attested_head_sha: review.fresh_attested_head_sha }, nextTransition: next, now: now() }); }
      else if (planned.transition === "REVISE") { const authority = await deps.attestRevisionAuthority({ runId: options.runId, stateDirectory: options.stateDirectory }); ledger = await checkpointAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, transition: "REVISE", payload: revisionOrchestrationPayload(authority), now: now() }); activeAttemptId = ledger.current_attempt!.attempt_id; const revision = await deps.reviseRun({ runId: options.runId, revisionRound: authority.revisionRound, stateDirectory: options.stateDirectory, configPath: options.configPath, now }); assertRevisionResultForOrchestration(options.runId, revision, authority); ledger = await completeAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, attemptId: activeAttemptId, result: { state: revision.state, revision_round: revision.revision_round, previous_pr_head_sha: revision.previous_pr_head_sha, new_published_commit_sha: revision.new_published_commit_sha, remote_branch_sha: revision.remote_branch_sha, pull_request_number: revision.pull_request_number, result_bundle_sha256: revision.result_bundle_sha256, result_manifest_sha256: revision.result_manifest_sha256, next_review_round: revision.next_review_round }, nextTransition: "WAIT_WEB_VERDICT", usage: revisionOrchestrationUsage(revision), now: now() }); }
    } catch (error) { const current = await ensureRunLedger(options.stateDirectory, options.runId, now()); if (activeAttemptId && current.current_attempt?.status === "STARTED" && current.current_attempt.attempt_id === activeAttemptId) { const retryAfterMs = errorRetryAfterMs(error); ledger = await failAttempt({ stateDirectory: options.stateDirectory, runId: options.runId, attemptId: activeAttemptId, failureCode: errorCode(error), message: errorMessage(error), ...(retryAfterMs !== undefined ? { minimumRetryDelayMs: retryAfterMs } : {}), now: now() }); operationFailed = true; } else throw error; }
    const afterSnapshot = await deps.readSnapshot(options.stateDirectory, options.runId); const afterPlan = deriveNextTransition(afterSnapshot); const blockedByFailure = operationFailed || ["BLOCKED", "PAUSED", "FAILED"].includes(ledger.status); const needsInput = !blockedByFailure && afterPlan.transition === "REGISTER_WEB_PACK" ? "web_pack_path" : !blockedByFailure && afterPlan.transition === "WAIT_WEB_VERDICT" ? "web_verdict_path" : null; return { ledger, planned: afterPlan, progressed: !blockedByFailure, needs_input: needsInput };
  });
}
