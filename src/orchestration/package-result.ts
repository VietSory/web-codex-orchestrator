...
async function prepareResultGitRuntime(stateDirectory: string): Promise<string> {
  ...
  if (hooksStat.isSymbolicLink() || !hooksStat.isDirectory() || await fs.realpath(hooks) !== hooks) throw new OrchestrationError("ORCHESTRATION_RESULT_GIT_FAILED", "Result Bundle Git hooks directory is unsafe.");
  const emptyConfig = path.join(runtime, "empty-config");
  try { await fs.writeFile(emptyConfig, "", { flag: "wx", mode: 0o600 }); }
  catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error;
    const configStat = await fs.lstat(emptyConfig);
    if (configStat.isSymbolicLink() || !configStat.isFile() || configStat.size !== 0) throw new OrchestrationError("ORCHESTRATION_RESULT_GIT_FAILED", "Result Bundle empty Git config is unsafe.");
  }
  return runtime;
}
...
async function createGitRunner(stateDirectory: string, limits: ResultBundleLimits): Promise<GitRunner> {
  ...
  return new SecureGitRunner(process.env, runtime, undefined, { stdoutMaxBytes: maximumEvidenceBytes, stderrMaxBytes: Math.min(1_048_576, maximumEvidenceBytes) });
}
...
function tokenCounter(value: unknown, label: string): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `Executor review evidence has invalid ${label}.`);
  return value as number;
}

async function readReviewUsage(options: { executorDirectory: string; reviewer: "terra" | "sol"; round: number; evidenceSha256: string | null }): Promise<{ input_tokens: number; cached_input_tokens: number; output_tokens: number }> {
  if (options.round === 0 && options.evidenceSha256 === null) return { input_tokens: 0, cached_input_tokens: 0, output_tokens: 0 };
  if (options.evidenceSha256 === null || options.round < 1) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `${options.reviewer} review usage has inconsistent round/evidence identity.`);
  const evidencePath = path.join(options.executorDirectory, "evidence", `${options.reviewer}-${options.round}-${options.evidenceSha256}.json`);
  const bytes = await readStableExecutorStateFile(evidencePath, MAX_EXECUTOR_EVIDENCE_BYTES);
  if (sha256(bytes) !== options.evidenceSha256) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `${options.reviewer} evidence changed after executor attestation.`);
  let parsed: unknown;
  try { parsed = JSON.parse(bytes.toString("utf8")); }
  catch { throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `${options.reviewer} evidence is not valid JSON.`); }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `${options.reviewer} evidence must be an object.`);
  const usage = (parsed as Record<string, unknown>).usage;
  if (usage === null || usage === undefined) return { input_tokens: 0, cached_input_tokens: 0, output_tokens: 0 };
  if (typeof usage !== "object" || Array.isArray(usage)) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", `${options.reviewer} evidence usage must be an object or null.`);
...
export async function packageResultForRun(options: { runId: string; stateDirectory: string; configPath: string; now?: () => Date }): Promise<ResultBundleReceipt> {
  ...
  let token: string;
  try { token = await resolveGitHubToken(github.authentication); }
  catch { throw new OrchestrationError("ORCHESTRATION_RESULT_AUTH_UNAVAILABLE", "GitHub credentials are unavailable."); }
  ...
  const ready = await attestReadyExecutorSnapshot({ runId: options.runId, artifactSha256: selected.artifact_sha256, stateDirectory: options.stateDirectory, configPath: options.configPath });
  const executor = ready.receipt;
  const run = ready.source.trusted.runReceipt;
  if (executor.change_set_digest === null || executor.change_set_digest !== ready.changeSetDigest) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", "READY executor receipt lost its exact change-set binding before Result packaging.");
  ...
  if (!executorReceiptBytes.equals(canonicalExecutorReceipt)) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", "Executor receipt changed after the READY snapshot was attested.");

  // A verified repair changes the executor receipt/digest after an earlier Result
  // Bundle was READY. Preserve that stale generation before Phase 6 runs so its
  // normal ready short-circuit cannot return or reject the old authority.
  await retireStaleResultGeneration({
    stateDirectory: options.stateDirectory,
    runId: options.runId,
    executorReceiptSha256: sha256(executorReceiptBytes),
  ...
  const executionEvidence: Record<string, unknown> = {
    ...
    implementer: { model: "web-implementation-pack", reasoning_effort: "not-applicable", iterations: 0, thread_id: "" },
  ...
  const limits: ResultBundleLimits = { ...DEFAULT_RESULT_BUNDLE_LIMITS, ...(config.result_bundle ?? {}) };
  ...
  const args: Parameters<typeof packageResultBundle>[0] = {
    ...
    secrets: [token],
    authority: { executionEvidence, executionReceiptBytes: executorReceiptBytes, publishReceiptPath: path.join(publishDirectory, "git-publish.json"), draftReceiptPath: path.join(publishDirectory, "github-draft-pr.json") },
  };
  ...
  if (result.execution_receipt_sha256 !== sha256(executorReceiptBytes) || result.change_set_sha256 !== ready.changeSetDigest || result.base_commit !== run.base_commit) throw new OrchestrationError("ORCHESTRATION_RESULT_AUTHORITY_DRIFT", "Result Bundle did not bind the exact selected executor authority.");
  return result;
}
