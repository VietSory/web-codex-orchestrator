...
export function validateExecutionContract(manifest: unknown): ExecutionValidationReport {
  ...
  if (!isRecord(manifest) || manifest.schema_version !== "1.2") {
    add(issues, "EXECUTION_CONTRACT_REQUIRED", "Only schema 1.2 bundles may be prepared.");
  ...
  if (!isRecord(delivery)) {
    add(issues, "DELIVERY_CONTRACT_INVALID", "delivery is required for schema 1.2.");
  } else {
    ...
    if (!Array.isArray(delivery.push_after)) {
      ...
      if (gates.some((gate) => typeof gate !== "string" || !KNOWN_PUSH_GATES.has(gate))) {
        add(issues, "DELIVERY_CONTRACT_INVALID", "delivery.push_after contains an unknown gate.");
      }
  ...
  if (!isRecord(policy)) {
    add(issues, "GIT_POLICY_INVALID", "git_policy is required for schema 1.2.");
  } else {
  ...
  if (isRecord(delivery) && isRecord(policy) && typeof delivery.branch_name === "string" && Array.isArray(policy.deny_direct_push_branches) && policy.deny_direct_push_branches.includes(delivery.branch_name)) add(issues, "BRANCH_POLICY_VIOLATION", "branch_name is denied by git policy.");

  if (issues.length > 0) return { ok: false, issues };
  const result = manifest as unknown as BundleManifest & { schema_version: "1.2" };
