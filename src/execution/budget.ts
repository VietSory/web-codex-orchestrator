...
export interface Usage { implementationIterations: number; internalReviewRounds: number; solReviewRounds: number; totalTurns: number; inputTokens: number; outputTokens: number; startedAt: number; }

export class BudgetTracker {
  readonly usage: Usage;
  constructor(private readonly limits: AgentLimits, startedAt = Date.now(), initial?: Partial<Usage>) {
    this.usage = { implementationIterations: initial?.implementationIterations ?? 0, internalReviewRounds: initial?.internalReviewRounds ?? 0, solReviewRounds: initial?.solReviewRounds ?? 0, totalTurns: initial?.totalTurns ?? 0, inputTokens: initial?.inputTokens ?? 0, outputTokens: initial?.outputTokens ?? 0, startedAt };
  }
  ...
  recordTokens(inputTokens: number | undefined, outputTokens: number | undefined): void { this.usage.inputTokens += inputTokens ?? 0; this.usage.outputTokens += outputTokens ?? 0; if (this.usage.inputTokens > this.limits.maximum_total_input_tokens || this.usage.outputTokens > this.limits.maximum_total_output_tokens) throw new ExecutionError("BUDGET_EXHAUSTED", "Configured token budget is exhausted."); }
}
