...
const transitions: Record<ExecutionState, readonly ExecutionState[]> = {
  ...
  // A normal task has one selected reviewer. Terra-selected runs take the
  // historical VERIFYING -> TERRA_REVIEWING edge; Sol-selected runs go
  // directly VERIFYING -> SOL_REVIEWING without manufacturing a Terra review.
  VERIFYING: ["TERRA_REVIEWING", "SOL_REVIEWING", "TERRA_FIXING", "VERIFICATION_FAILED", "BUDGET_EXHAUSTED", "FAILED", "INTERRUPTED"],
  TERRA_FIXING: ["TERRA_IMPLEMENTING", "BUDGET_EXHAUSTED", "AGENT_FAILED", "FAILED", "INTERRUPTED"],
  // Terra-selected runs may finish directly after Terra APPROVE. Legacy
  // dual-review automation can still continue to SOL_REVIEWING.
  TERRA_REVIEWING: ["SOL_REVIEWING", "READY_FOR_PUBLISH", "TERRA_FIXING", "WEB_REVIEW_REQUIRED", "HUMAN_REQUIRED", "AGENT_FAILED", "BUDGET_EXHAUSTED", "FAILED", "INTERRUPTED"],
