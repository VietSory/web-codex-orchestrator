...
import { DEFAULT_REVIEWER, parseReviewerSelection, type ReviewerSelection } from "../agent/reviewer-selection.js";
...
export async function writeReviewMode(stateDirectory: string, selection: ReviewerSelection, now: () => Date = () => new Date()): Promise<void> {
  ...
  const value: StoredReviewMode = {
    schema_version: "1.0",
    reviewer: normalized.kind,
