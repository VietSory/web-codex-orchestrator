...
A process can die after an external side effect has committed but before the orchestration ledger records completion. On restart WCO checks the still-`STARTED` attempt before planning another action. It adopts a Phase 9 registration only when a selection created no earlier than the active attempt reconstructs the same sealed archive/pack request; a selection from a prior Web round is ignored as stale history. It adopts a terminal Phase 10 result only after canonical run/registration/worktree/transaction/evidence attestation. It adopts publication only when the persisted publish receipt is `PUSHED`, binds the exact run/base/delivery target/change-set/changed paths, has `remote_branch_sha === commit_sha`, and the READY executor digest still re-attests. Any current-result identity mismatch fails closed as `ORCHESTRATION_RECOVERY_CONFLICT`. Incomplete or stale evidence is not treated as success; the existing idempotent phase primitive remains responsible for resuming its own incomplete operation.
