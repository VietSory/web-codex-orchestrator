...
6. `WEB-REVIEW-CONTRACT.md`, review policy, verdict schema, revision-request schema, spec lock, and reviewed-entry set are hash-bound through the exact verified Result Bundle manifest and Phase 6 receipt.
7. The exact embedded verdict schema is authoritative for the reviewed bundle. The built-in validator remains a minimum hard floor, so neither side can silently weaken validation.
8. The untrusted Web verdict is a regular non-symlink file with an allocation-safe 1 MiB cap. Phase 7 reads only the pre-attested file size, probes for growth, verifies stable identity/size/metadata across the file-handle read, canonicalizes JSON, and applies the same cap to canonical bytes.
9. Terminal round retries are idempotent only when verdict, decision event, and (for `REVISE`) revision request still exist and hash exactly to the terminal receipt. A retry must also re-run frozen-policy validation and obtain a **fresh GitHub attestation**; a previously terminal receipt never authorizes a stale merge prompt after repository or PR drift.
10. Every Phase 7 lifecycle directory under `handoff/reviews/` must be a real directory inside the configured state root. Existing symbolic-link ancestors, symbolic-link artifacts, special files, and real-path escapes fail closed. Canonical artifact reads and compare-and-adopt operations never follow symlink targets.
11. Persisted Phase 7 review files are allocation-bounded to 2 MiB and must retain stable file identity/size/metadata across reads. Receipt warnings/errors are structurally bounded; repeated failures retain at most 32 bounded error diagnostics instead of growing state indefinitely.
12. The per-round lock is create-only and owner-bound by PID plus nonce. Phase 7 never automatically steals, unlinks, or replaces an existing live, stale, or malformed lock because path-based stale-lock takeover has a replacement race. A stale/malformed lock requires explicit operator recovery after verifying no live owner exists.
13. Fresh GitHub attestation requires the exact PR number, **Draft** state, open/unmerged state, head/base repository identity, head/base branch, head SHA, and base SHA. Production GitHub reads are pinned to `api.github.com`, have a 10-second timeout, and are incrementally capped at 1 MiB even when `Content-Length` is absent or incorrect.
14. GitHub failures retain stable semantics: authentication/authorization failures are `WEB_REVIEW_AUTH_ERROR`, transient service/rate-limit/transport failures are `WEB_REVIEW_NETWORK_ERROR`, and missing or invalid PR identity is `WEB_REVIEW_REPOSITORY_DRIFT`. All fail closed before terminal dispatch.
15. Phase 7 performs no GitHub mutation. Marking a PR Ready and merging remain human-controlled actions outside Phase 7. An `APPROVED` receipt is authority only for its exact attested head SHA; any later head or PR-state change requires fresh validation before merge.
