...
test("V04-AUTO-006 restart while waiting for mandatory Web final review reuses the durable review job", async () => {
  ...
  let reviewCreates = 0;
  let firstPolls = 0;
  const firstBridge = { async waitForVerdict() { firstPolls += 1; return null; } } as unknown as WebBridge;
  try {
    ...
    const secondBridge = { async waitForVerdict(jobId: string) { assert.equal(jobId, "review-job-1"); return { review_id: "approved-resume" } as never; } } as unknown as WebBridge;
