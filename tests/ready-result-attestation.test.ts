...
function publishReceipt(): GitPublishReceipt {
  return {
    ...
    branch_name: "codex/ready-result",
    remote_name: "origin",
    ...
    remote_branch_sha: commitSha,
    created_at: "2026-08-09T00:00:00.000Z",
    updated_at: "2026-08-09T00:00:01.000Z",
    committed_at: "2026-08-09T00:00:00.500Z",
    pushed_at: "2026-08-09T00:00:01.000Z",
  };
}
...
function draftReceipt(overrides: Partial<DraftPullRequestReceipt> = {}): DraftPullRequestReceipt {
  return {
    ...
    base_branch: "main",
    head_branch: "codex/ready-result",
    ...
    pull_number: 42,
    pull_url: "https://github.com/example/my.repo/pull/42",
    ...
    created_at: "2026-08-09T00:00:00.000Z",
    updated_at: "2026-08-09T00:00:01.000Z",
    create_attempted_at: "2026-08-09T00:00:00.500Z",
...
function resultReceipt(): ResultBundleReceipt {
  return {
    result_bundle_version: "1.1",
    run_id: runId,
    state: "READY_FOR_WEB_REVIEW",
    input_digest_sha256: "3".repeat(64),
    execution_receipt_sha256: "4".repeat(64),
    git_publish_receipt_sha256: "5".repeat(64),
    draft_pr_receipt_sha256: "6".repeat(64),
    accepted_bundle_tree_sha256: "7".repeat(64),
    change_set_sha256: changeSet,
    base_commit: baseCommit,
    ...
    pull_request: {
      number: 42,
      url: "https://github.com/example/my.repo/pull/42",
      state: "open",
      draft: true,
      head_branch: "codex/ready-result",
      head_sha: commitSha,
    ...
    archive_relative_path: "handoff/result.zip",
    ...
    created_at: "2026-08-09T00:00:00.000Z",
    updated_at: "2026-08-09T00:00:01.000Z",
    built_at: "2026-08-09T00:00:01.000Z",
    verified_at: "2026-08-09T00:00:01.000Z",
    ready_at: "2026-08-09T00:00:01.000Z",
    spec_set_sha256: "a".repeat(64),
    review_contract_sha256: "b".repeat(64),
    review_policy_sha256: "c".repeat(64),
    verdict_schema_sha256: "d".repeat(64),
    revision_request_schema_sha256: "e".repeat(64),
...
function freshPr(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    number: 42,
    html_url: "https://github.com/example/my.repo/pull/42",
    state: "open",
    draft: true,
    merged: false,
    merged_at: null,
    title,
    head: { ref: "codex/ready-result", sha: commitSha },
    base: { ref: "main" },
    ...overrides,
  };
}
...
test("READY-RESULT-001 fresh retry re-attests the exact dotted GitHub repository identity", async (t) => {
  ...
  await reattestReadyResultBundleAuthority({ stateDirectory: state, runId, receipt: resultReceipt(), githubClient: github });
...
test("READY-RESULT-002 fresh retry rejects a PR that is no longer Draft", async (t) => {
  ...
  await assert.rejects(
    () => reattestReadyResultBundleAuthority({ stateDirectory: state, runId, receipt: resultReceipt(), githubClient: github }),
...
test("READY-RESULT-004 persisted repository identity mismatch fails before GitHub API access", async (t) => {
  const state = await fixture(t, draftReceipt({ repository_name: "other.repo" }));
  const github = new FakeGitHub();
  await assert.rejects(
    () => reattestReadyResultBundleAuthority({ stateDirectory: state, runId, receipt: resultReceipt(), githubClient: github }),
