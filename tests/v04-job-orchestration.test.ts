...
test("V04-AUTO-001 AUTOPILOT reuses execution/publish/revision services through Web approval", async () => {
  ...
  try {
    ...
    assert.deepEqual(calls, [
      "execute", "publish", "draft", "package",
      "web-job", "verdict", "attest-revision", "revise",
      "web-job", "verdict",
    ]);
...
test("V04-AUTO-002 execution replan/human boundaries stop instead of weakening the contract", async () => {
  ...
  const execution = {
    ...
    errors: [{ code: "REPLAN_REQUIRED", message: "Repository facts conflict with the frozen plan." }],
  ...
  try {
    const receipt = await driveAutopilotJob({
      ...
      dependencies: {
        ...
        publish: async () => { published = true; return PUSHED; },
      },
    });
