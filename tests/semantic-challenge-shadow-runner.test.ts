...
class EvidenceDrivenTransport implements SemanticChallengeTransport {
  ...
  constructor(
    private readonly request: ReturnType<typeof createSemanticChallengeRequest>,
  ...
  async waitForSemanticChallengeAction(_jobId: string, afterSequence: number): Promise<SemanticChallengeRemoteAction | null> {
    ...
    const result = rawReadResult(this.delivered.result);
    ...
    this.sealed = {
      schema_version: "1.0", kind: "semantic_understanding_sealed", challenge_id: this.request.challenge_id, repository: this.request.repository,
  ...
  async submitSemanticChallengeRepositoryResult(_jobId: string, result: RepositoryCommandResult) {
    if (this.stateDirectory) {
      const recovered = await readLatestTrajectoryBoundSemanticChallengeEvidence({ stateDirectory: this.stateDirectory, request: this.request });
      assert.equal(recovered?.observation_count, 1, "evidence must be durable before provider result delivery");
    }
...
test("shadow runner owns evidence, durably snapshots it before provider delivery, and seals a digest trajectory", async (t) => {
  const value = await fixture(); t.after(async () => { await rm(value.root, { recursive: true, force: true }); });
  const transport = new EvidenceDrivenTransport(value.request, value.stateDirectory);
  ...
  assert.deepEqual((await readSemanticChallengeTrajectory({ stateDirectory: value.stateDirectory, request: value.request })).map((receipt) => receipt.event_type), ["challenge_created", "repository_observation", "understanding_sealed"]);
  const recovered = await readLatestTrajectoryBoundSemanticChallengeEvidence({ stateDirectory: value.stateDirectory, request: value.request });
  assert.equal(recovered?.observation_count, 1);
  assert.equal(JSON.stringify(recovered).includes("currentFocus"), false, "durable evidence snapshot must remain source-byte stripped");
});
...
test("shadow runner serializes a challenge before any duplicate provider side effect", async (t) => {
  ...
  await assert.rejects(runSemanticChallengeShadow({ transport: secondTransport, request: value.request, repositoryPath: value.repositoryPath, stateDirectory: value.stateDirectory }), /execution is already active/i);
  assert.equal(duplicateProviderCreates, 0);
  releaseFirstResolve();
...
test("partial durable trajectory fails closed instead of pretending provider replay is exact", async (t) => {
  const value = await fixture(); t.after(async () => { await rm(value.root, { recursive: true, force: true }); });
