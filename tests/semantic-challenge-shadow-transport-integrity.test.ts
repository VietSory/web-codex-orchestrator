...
async function fixture() {
  const root = await mkdtemp(path.join(os.tmpdir(), "wco-shadow-transport-integrity-"));
  ...
  const request = createSemanticChallengeRequest({
    challengeId: "challenge-transport-integrity",
    repository: { repository_id: "repo-transport-integrity", base_branch: "main", base_commit: baseCommit },
    originalGoal: "Keep transport mutation outside durable challenge provenance.",
  });
...
test("transport cannot mutate the runner request, job id, request id, or seal envelope across await boundaries", async (t) => {
  ...
  let providerSeal: SemanticUnderstandingEnvelope | null = null;
  const transport: SemanticChallengeTransport = {
    ...
    async receiveSemanticUnderstanding(jobId) {
      seenJobIds.push(jobId);
      return providerSeal ? structuredClone(providerSeal) : null;
    },
  };
  ...
  assert.deepEqual(new Set(seenJobIds), new Set(["stable-job"]));
...
test("seal fails before provider understanding acceptance when latest durable evidence disappears after delivery", async (t) => {
  const value = await fixture();
  t.after(async () => { await rm(value.root, { recursive: true, force: true }); });
  let delivered: RepositoryCommandResult | null = null;
  let receiveCalls = 0;
  const transport: SemanticChallengeTransport = {
    async createSemanticChallengeJob() { return identity("durable-seal-job"); },
  ...
  assert.equal(receiveCalls, 0, "durable evidence failure must stop before accepting provider-sealed understanding");
});
