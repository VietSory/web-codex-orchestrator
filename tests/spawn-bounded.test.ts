...
import { spawnBounded } from "../src/runtime/spawn-bounded.js";
...
test("SPAWN-BOUND-001 retains only the exact tail after large streamed output", async () => {
  ...
  assert.equal(result.stdoutBuffer?.byteLength, retainedBytes);
  assert.equal(result.stdout, "a".repeat(retainedBytes));
...
test("SPAWN-BOUND-002 zero-byte retention still counts output without retaining it", async () => {
  ...
  assert.equal(result.stdoutBuffer?.byteLength, 0);
});
