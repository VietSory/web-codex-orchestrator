...
test("v0.2 executor reserves each review turn before calling the provider and blocks the next call at the hard turn limit", async (t) => {
  ...
  const reviewer: ExecutorReviewerPort = {
    budget_policy: { maximum_model_turns: 1, maximum_elapsed_ms: 60_000 },
    async review(request) {
      calls += 1;
      return {
        verdict: "APPROVE",
        evidence: { reviewer: request.reviewer },
        usage: { model_turns: 0, input_tokens: 10, output_tokens: 2 },
      };
    },
  };
...
test("v0.2 executor keeps reserved turns and measured tokens separate without double-counting", async (t) => {
  const { fixture, registration } = await setup(t);
  const reviewer: ExecutorReviewerPort = {
    budget_policy: { maximum_model_turns: 2, maximum_elapsed_ms: 60_000 },
    async review(request) {
      return {
        verdict: "APPROVE",
        evidence: { reviewer: request.reviewer },
        usage: request.reviewer === "terra"
          ? { model_turns: 0, input_tokens: 11, output_tokens: 3 }
          : { model_turns: 0, input_tokens: 13, output_tokens: 5 },
      };
    },
  };
