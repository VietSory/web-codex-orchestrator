...
async function startSession(bridge: InspectingBridge, base: string, stateDirectory: string) {
  return await startLocalAuthoring({
    bridge,
...
test("runtime shadow persists only after the exact repository result has been delivered to Web", async () => {
  ...
  try {
    ...
    const current = await startSession(bridge, base, stateDirectory);
    receiptPath = semanticShadowReceiptPath({
      stateDirectory,
      repositoryId: "repo",
