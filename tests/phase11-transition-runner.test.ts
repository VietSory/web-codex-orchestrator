...
test("P11-RUN-001 continue advances REGISTER -> EXECUTE -> PUBLISH with one sealed attempt each", async (t) => {
  ...
  const deps = {
    ...
    async executePack() {
      ...
      return { state: "READY_FOR_PUBLISH", change_set_digest: "d".repeat(64), artifact_sha256: ARTIFACT } as never;
    },
    ...
    async publishReadyExecutor() {
      ...
      return { state: "PUSHED", commit_sha: "3".repeat(40), remote_branch_sha: "3".repeat(40) } as never;
    },
