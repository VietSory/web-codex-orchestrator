...
- The durable run ledger is not trusted merely because it is parseable JSON. Every read validates run identity, status/pause consistency, transition and current-attempt lifecycle, retry/circuit enums and timing, per-transition/total-attempt accounting, model/token budget semantics, bounded diagnostics/events and the compacted event hash chain. Typed-looking but semantically contradictory state fails closed as `ORCHESTRATION_STATE_INVALID` before orchestration authority is consumed.
